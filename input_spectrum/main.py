...
def create_model(opt):
    if opt.netType == 'EnvNet5':
        model = models.EnvNet5(opt.nClasses)
    elif opt.netType == 'EnvNet6':
        model = models.EnvNet6(opt.nClasses)
    return model
        ...
        for epoch in range(opt.nEpochs):
            ...
            with torch.no_grad():
                for data in validation_generator:
                    ...
                    # input_array = input_array.reshape(input_array.shape[0]*opt.nCrops, input_array.shape[2], input_array.shape[3])
                    # inputs = input_array[:,None,:,:].to(device)

                    input_array = input_array.reshape(input_array.shape[0] * opt.nCrops, input_array.shape[2],
                                                      input_array.shape[3], input_array.shape[4])
                    inputs = input_array.to(device)
            ...
            print('Elapsed [{}], Epoch [{}/{}], Train_loss = {:0.4f}, Val_loss = {:0.4f}, Accuracy = {}'.format(
            ...
            if (epoch+1) in opt.save_model or (epoch+1) == opt.nEpochs:
                torch.save(model.state_dict(),
                           os.path.join(opt.save_dir, "model_{}_{}_{}.bin".format(epoch+1, test_fold, accuracy)))
